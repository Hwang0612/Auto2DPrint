...
if run_clicked and pdf_file is not None:
    ...
    with result_container:
        ...
        st.code(
            ...
            f"    Scale: {m.get('drawing_scale','?')} (method: {m.get('scale_method','?')}"
            f"{f', confidence: {m["scale_confidence"]:.0%}, {m["scale_n_samples"]} samples' if m.get('scale_confidence') else ''})"
            f"\n"
