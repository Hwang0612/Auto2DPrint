...
with st.sidebar:
    ...
    with st.expander("🛠️ Parametric Parameters", expanded=True):
        ...
        wall_regex = st.text_input("Wall layer CAD pattern", value=".*wall.*", help="Regex pattern matching CAD wall layer name (e.g. 'Wall', '2D Walls', 'A-WALL')")
...
with tab_pipeline:
    ...
    if "fallback_note" in stage1.meta:
        st.caption(f"ℹ️ **M1 Layer Ingestion Rule**: {stage1.meta['fallback_note']} (Matched layers: `{', '.join(stage1.meta.get('matched_layers', []))}`)")
...
with tab_3d:
    ...
    with c_mode:
        render_mode = st.selectbox("3D Render Mode", options=["Extruded Concrete Beads", "Wireframe Toolpath"], index=0)
    with c_color:
    ...
    with subtab_2d:
        ...
        with col_ctrl:
            selected_layer_idx = st.number_input("Select Layer Index", min_value=0, max_value=max_l, value=0, step=1)
    ...
    with subtab_anim:
        ...
        with col_ctrl_anim:
            selected_anim_layer = st.number_input("Simulation Layer Index", min_value=0, max_value=max_l, value=0, step=1, key="anim_layer")
            st.caption("Click ▶ Play inside the 3D canvas toolbar to animate the robotic nozzle extruding concrete live in 3D space.")
            
        with col_fig_anim:
            fig_anim = build_3d_nozzle_anim(stage3, layer_idx=selected_anim_layer, bbox=stage3.coordinate_frame.page_bbox_world_mm)
