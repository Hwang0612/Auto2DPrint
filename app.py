...
st.set_page_config(page_title="PrintPlan AI", page_icon="🏗️", layout="wide")
...
with st.sidebar:
    ...
    scale_auto = st.checkbox("Auto-detect scale from PDF", value=True)
    scale = st.number_input("Drawing scale (1:N)", value=50, min_value=1, step=1, disabled=scale_auto)
    ...
    st.divider()
    st.caption("Auto2DPrint AI v1.3")
...
with col_upload:
    st.subheader("Upload")
    pdf_file = st.file_uploader("Drop your vector PDF floor plan here", type=["pdf"],
...
if run_clicked and pdf_file is not None:
    ...
    with result_container:
        ...
        # M1
        stage1 = parse_pdf(pdf_path, M1Params(drawing_scale=None if scale_auto else scale))
        m = stage1.meta
