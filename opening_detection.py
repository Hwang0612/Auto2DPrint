...
v3.0 — Reads 'Doors' and 'Windows' OCG layers directly using PyMuPDF.
        Falls back to pdfplumber gap detection"""
opening_detection.py
====================
PrintPlan AI — Opening Detection & Z-Aware Toolpath Module
-----------------------------------------------------------
...
class FloorPlanParser:
    ...
    def _detect_from_ocg_layers(self, doc, page, paths, available_layers) -> List[Opening]:
        ...
        if "Doors" in available_layers:
            ...
            # Collect arc groups: each path object with Bézier curves = one door swing
            arc_groups = []
            for p in door_paths:
            ...
            for arc_curves in arc_groups:
                ...
                # ── Fix: door width ──────────────────────────────────────────
                # For a standard 90° door arc, chord = radius × √2.
                # The true door width equals the arc radius, so divide by √2.
                ...
                # the 90° arc — either (ex, hy) or (hx, ey).
...
if __name__ == "__main__":
    ...
    for o in ops:
        print(f"  {o.summary()}") for PDFs without named layers.
    ...
    def _detect_from_ocg_layers(self, doc, page, paths, available_layers) -> List[Opening]:
        ...
        if "Doors" in available_layers:
            ...
            for arc_curves in arc_groups:
                ...
                hx, hy   = to_world(hinge_pt.x, hinge_pt.y)
                ex, ey   = to_world(end_pt.x, end_pt.y)
                leaf_w   = math.hypot(ex - hx, ey - hy)

                # Find the 2 nearest jamb lines to this hinge
                nearby = sorted(
                    jamb_lines,
                    key=lambda j: min(
                        math.hypot(j["p1"][0] - hx, j["p1"][1] - hy),
                        math.hypot(j["p2"][0] - hx, j["p2"][1] - hy),
                    )
                )[:2]

                if nearby:
                    mids = [((j["p1"][0]+j["p2"][0])/2, (j["p1"][1]+j["p2"][1])/2)
                            for j in nearby]
                    cx = sum(m[0] for m in mids) / len(mids)
                    cy = sum(m[1] for m in mids) / len(mids)
                    j0 = nearby[0]
                    dx = abs(j0["p2"][0] - j0["p1"][0])
                    dy = abs(j0["p2"][1] - j0["p1"][1])
                    wall_ori = "vertical" if dy > dx else "horizontal"
                    # Jamb lines run perpendicular to the wall → wall_dir is opposite
                    wall_dir_label = "vertical" if dy > dx else "horizontal"
                else:
                    cx = (hx + ex) / 2; cy = (hy + ey) / 2
                    wall_dir_label = "vertical" if abs(ey-hy) > abs(ex-hx) else "horizontal"
        ...
        if "Windows" in available_layers:
            ...
            for cluster_pts in clusters:
                ...
                cluster_segs = [s for s in win_segs if near(s)]
                if cluster_segs:
                    longest = max(cluster_segs, key=lambda s: s["len"])
                    width   = longest["len"]
                    ln_dx   = abs(longest["p2"][0] - longest["p1"][0])
                    ln_dy   = abs(longest["p2"][1] - longest["p1"][1])
                    wall_ori = "vertical" if ln_dy > ln_dx else "horizontal"
                else:
                    span_x = max(xs) - min(xs); span_y = max(ys) - min(ys)
                    width   = max(span_x, span_y)
                    wall_ori = "vertical" if span_y > span_x else "horizontal"
